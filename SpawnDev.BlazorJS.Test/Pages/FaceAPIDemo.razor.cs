...
namespace SpawnDev.BlazorJS.Test.Pages
{
    ...
    {
        [Inject]
        MediaDevicesService _mediaDevicesService { get; set; } = default!;
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender)
            {
                if (!_beenInit)
                {
                    ...
                    _cameraCanvasProcessedElCtx = _cameraCanvasProcessedEl.Get2DContext(new CanvasRenderingContext2DSettings { WillReadFrequently = true });
                    _ = RefreshCameraList();
