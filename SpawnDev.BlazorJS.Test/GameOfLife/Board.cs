...
namespace SpawnDev.BlazorJS.Test.GameOfLife
{
    public class Board
    {
        ...
        public double StartingPopulationPercent { get; private set; } = 50;
            ...
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    ...
                    if (selfValueNew != selfValue)
                    {
                        if (OnChanged != null)
                        {
                            OnChanged.Invoke(new ChangedCell { X = x, Y = y, Value = selfValueNew });
                        }
                        else
                        {
        ...
        public event Action<ChangedCell> OnChanged = default!;
        public void Resize(int width, int height, double startingPopulationPercent)
        {
            Width = width;
            Height = height;
            Reset(startingPopulationPercent);
        }
        ...
        public void Reset() => Reset(StartingPopulationPercent);
        /// <summary>
        /// 
        /// </summary>
        /// <param name="startingPopulationPercent"></param>
        public void Reset(double startingPopulationPercent)
        {
            StartingPopulationPercent = startingPopulationPercent;
            Generation = 0;
            ...
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    var occupied = NextBool(startingPopulationPercent);
